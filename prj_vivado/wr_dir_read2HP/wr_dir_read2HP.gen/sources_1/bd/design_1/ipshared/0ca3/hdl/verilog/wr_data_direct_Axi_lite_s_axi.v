...
// ==============================================================
`timescale 1ns/1ps
module wr_data_direct_Axi_lite_s_axi
#(parameter
...
//------------------------Parameter----------------------
localparam
    ADDR_WIDTH_IMG_DATA_0  = 6'h10,
    ADDR_WIDTH_IMG_CTRL    = 6'h14,
    ADDR_BASE_ADDR_DATA_0  = 6'h18,
    ADDR_BASE_ADDR_CTRL    = 6'h1c,
    ADDR_STATISTICS_DATA_0 = 6'h20,
    ADDR_STATISTICS_CTRL   = 6'h24,
    WRIDLE                 = 2'd0,
    WRDATA                 = 2'd1,
    WRRESP                 = 2'd2,
    WRRESET                = 2'd3,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_BASE_ADDR_DATA_0: begin
                    rdata <= int_base_addr[31:0];
                end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_base_addr[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_BASE_ADDR_DATA_0)
            int_base_addr[31:0] <= (WDATA[31:0] & wmask) | (int_base_addr[31:0] & ~wmask);
    end
end
